...
export default async function TourismDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="dark bg-black pb-24 pt-28 text-zinc-100 md:pt-36">
    ...
    <div className="mx-auto mt-9 max-w-[1500px] px-5 md:px-10 lg:px-16"><div className="relative aspect-[16/8] overflow-hidden bg-gradient-to-br from-amber-200 via-orange-100 to-stone-200 shadow-lg shadow-stone-900/10 dark:from-amber-900 dark:to-zinc-800">{place.image_url ? <Image src={place.image_url} alt={place.name} fill priority unoptimized className="object-cover" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(255,255,255,.75),transparent_13%),linear-gradient(135deg,transparent_0_50%,rgba(120,53,15,.13)_50.5%_51%,transparent_51.5%)]" />}</div>{gallery.length > 1 && <div className="mt-3 grid grid-cols-4 gap-3">{gallery.slice(1, 5).map((image, index) => <div key={image} className="relative aspect-[4/3] overflow-hidden bg-stone-200 dark:bg-zinc-800"><Image src={image} alt={`${place.name} ${index + 2}`} fill unoptimized className="object-cover" /></div>)}</div>}<div className="mt-4 flex flex-wrap gap-3">{place.maps_url && <a href={place.maps_url} target="_blank" rel="noreferrer" className="bg-[#f4c300] px-4 py-2.5 text-sm font-semibold text-zinc-950 hover:bg-yellow-300">Buka peta</a>}{place.website_url && <a href={place.website_url} target="_blank" rel="noreferrer" className="border border-white/25 bg-transparent px-4 py-2.5 text-sm font-semibold text-white hover:bg-white hover:text-black">Website</a>}</div></div>

    <div className="mx-auto mt-10 grid max-w-[1500px] gap-10 px-5 md:px-10 lg:grid-cols-[minmax(0,1fr)_21rem] lg:px-16">
