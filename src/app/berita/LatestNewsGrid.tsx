    ...
    <>
      <div className="grid gap-x-8 sm:grid-cols-2">
        {visibleArticles.map((article) => (
          <article key={article.id} className="group border-b border-[#d7d1c6] py-5 first:pt-0 sm:[&:nth-child(2)]:pt-0">
            <Link href={`/${article.slug}`} className="grid grid-cols-[minmax(0,1fr)_112px] gap-4 sm:grid-cols-[minmax(0,1fr)_140px] sm:gap-5">
              <div className="min-w-0">
                <time className="text-[10px] font-bold uppercase tracking-[.15em] text-[#77736b]" dateTime={articleDate(article)}>{displayDate(articleDate(article))}</time>
                ...
                {article.excerpt ? <p className="mt-3 hidden line-clamp-2 text-xs leading-relaxed text-[#6d6961] lg:block">{article.excerpt}</p> : null}
              </div>
      ...
      {hasMore ? (
        <div className="mt-8 flex justify-center">
          <button
            ...
            className="rounded-[4px] border border-[#302f2c] px-5 py-2.5 text-xs font-bold uppercase tracking-[.1em] text-[#302f2c] transition hover:border-[#9b7513] hover:bg-[#f5c400] hover:text-black"
          >
            Load more news
          </button>
        </div>
