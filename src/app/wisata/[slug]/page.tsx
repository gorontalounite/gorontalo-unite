...
export default async function TourismDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const amenities = toList(details.amenities);
  ...
  const rating = toText(details.rating);
  const reviewCount = toText(details.review_count);
  ...
  return <main className="bg-[#fcfbf8] pb-20 text-slate-900 dark:bg-zinc-950 dark:text-zinc-100">
    <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-end">
        <div><p className="text-xs font-bold uppercase tracking-[.22em] text-amber-700 dark:text-amber-300">{category}{place.location ? ` · ${place.location}` : ""}</p><h1 className="mt-3 max-w-4xl font-display text-4xl font-bold leading-tight tracking-tight sm:text-6xl">{place.name}</h1><div className="mt-5 flex flex-wrap gap-2">{rating && <span className="rounded-full bg-amber-100 px-3 py-1.5 text-sm font-bold text-amber-900 dark:bg-amber-300 dark:text-zinc-950">★ {rating}{reviewCount ? ` · ${reviewCount} ulasan` : ""}</span>}{highlights.map((highlight) => <span key={highlight} className="rounded-full bg-white px-3 py-1.5 text-sm text-slate-600 shadow-sm ring-1 ring-black/5 dark:bg-zinc-900 dark:text-zinc-300 dark:ring-white/10">{highlight}</span>)}</div></div>
    ...
    <div className="mx-auto mt-10 grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-[minmax(0,1fr)_21rem] lg:px-8">
      <article className="min-w-0"><section><p className="text-xs font-bold uppercase tracking-[.2em] text-amber-700 dark:text-amber-300">Tentang tempat ini</p><div className="mt-4 space-y-5 text-[1.05rem] leading-8 text-slate-700 dark:text-zinc-300">{paragraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></section>
        ...
        {amenities.length > 0 && <section className="mt-10"><p className="text-xs font-bold uppercase tracking-[.2em] text-amber-700 dark:text-amber-300">Amenities</p><h2 className="mt-2 font-display text-3xl font-bold tracking-tight">Fasilitas yang tersedia</h2><ul className="mt-5 grid gap-3 sm:grid-cols-2">{amenities.map((amenity) => <li key={amenity} className="flex items-center gap-3 rounded-xl border border-stone-200 bg-white px-4 py-3 text-sm font-medium dark:border-zinc-800 dark:bg-zinc-900"><span className="grid h-6 w-6 place-items-center rounded-full bg-amber-100 text-xs text-amber-800 dark:bg-amber-300 dark:text-zinc-950">✓</span>{amenity}</li>)}</ul></section>}
