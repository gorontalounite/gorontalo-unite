...
export default async function BeritaPage({ searchParams }: PageProps) {
  ...
  return (
    <div className="dark min-h-screen bg-black text-white">
      <main className="mx-auto max-w-[1500px] px-5 pb-24 pt-28 md:px-10 md:pt-36 lg:px-16">
        ...
        {editorial && lead && <>
          <section aria-label="Sorotan hari ini" className="border-b border-white/20 pb-10"><div className="mb-7 flex items-end justify-between"><div><p className="text-[10px] font-bold uppercase tracking-[.28em] text-[#f4c300]">Kabar Pilihan</p><h1 className="mt-2 font-display text-5xl font-semibold uppercase leading-none tracking-[-.05em] md:text-7xl">Gorontalo<br />Hari Ini</h1></div><Link href="/berita" className="hidden text-xs font-bold uppercase tracking-[.18em] text-white/70 md:block">Seluruh berita →</Link></div><div className="grid gap-3 lg:grid-cols-[1.6fr_.8fr]"><NewsCard article={lead} variant="hero" /><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-1">{sideStories.map((article) => <NewsCard key={article.id} article={article} variant="compact" />)}</div></div></section>
        ...
        <section className={editorial ? "pb-16 pt-16" : "pb-16 pt-6"}><SectionHeading eyebrow={search ? "Hasil pencarian" : categoryLabel ? "Berita kategori" : "Berita & Informasi"} title={search ? "Ditemukan untuk Anda" : categoryLabel ? categoryLabel : "Latest News"} actionHref={!search && !categoryLabel ? "/berita" : undefined} actionLabel="See all" />
