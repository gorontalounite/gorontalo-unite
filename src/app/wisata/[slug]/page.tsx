...
export default async function TourismDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const categoryInfo = (categoryFields[category] ?? []).map(([key, label]) => ({ label, value: toText(details[key]) })).filter((item) => item.value);
