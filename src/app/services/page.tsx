...
export default async function ServicesPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto w-full px-4 sm:px-6 py-10 pb-24 md:pb-12">
      ...
      {/* Header */}
      <div className="mb-8">
        <span className="inline-block text-xs font-semibold uppercase tracking-widest text-brand dark:text-yellow-400 mb-3">
          Gorontalo Unite
        </span>
        ...
        {services.map((s) => (
          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-gray-900 dark:text-white group-hover:text-brand dark:group-hover:text-yellow-400 transition-colors">
                {s.title}
              </p>
            ...
            <svg className="w-4 h-4 text-gray-300 dark:text-zinc-600 flex-shrink-0 mt-0.5 group-hover:text-brand transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="mt-12">
        <div className="flex items-end justify-between mb-5">
          <div>
            <span className="text-xs font-semibold uppercase tracking-widest text-brand dark:text-yellow-400">Portofolio</span>
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mt-0.5">Karya & Proyek Kami</h2>
          </div>
          {portfolio.length > 0 && (
            <Link href="/portfolio" className="text-xs text-gray-400 hover:text-brand dark:hover:text-yellow-400 transition-colors">
              Lihat semua →
            </Link>
          )}
        </div>
        ...
        {portfolio.length > 0 ? (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {portfolio.map((item) => (
              <Link
                ...
                className="group rounded-2xl overflow-hidden border border-gray-100 dark:border-zinc-800 hover:border-brand/30 dark:hover:border-yellow-500/30 hover:shadow-md transition-all bg-white dark:bg-zinc-900"
              >
                <div className="aspect-square relative bg-gray-100 dark:bg-zinc-800 overflow-hidden">
                  {item.image_url ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-gradient-to-br from-yellow-50 to-teal-100 dark:from-yellow-950/30 dark:to-teal-950/30">
                      <span className="text-3xl">🌿</span>
                    </div>
                  )}
                </div>
                <div className="p-3">
                  <p className="text-xs font-semibold text-gray-800 dark:text-gray-200 line-clamp-2 leading-snug group-hover:text-brand dark:group-hover:text-yellow-400 transition-colors">
                    {item.title}
                  </p>
                  {item.excerpt && (
                    <p className="text-[10px] text-gray-400 dark:text-gray-500 mt-1 line-clamp-1">{item.excerpt}</p>
                  )}
                </div>
              </Link>
            ))}
          ...
          <div className="rounded-2xl border border-dashed border-gray-200 dark:border-zinc-700 py-10 text-center">
            <span className="text-4xl">🌿</span>
            <p className="text-sm text-gray-400 dark:text-gray-500 mt-3">Portofolio sedang disiapkan</p>
          </div>
        )}
      </div>
      ...
      <div className="mt-10 rounded-2xl border border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900 p-6 text-center">
        ...
        <a
          ...
          style={{ backgroundColor: "#F5C400", color: "#000000" }}
          className="inline-flex items-center gap-2 text-sm font-semibold px-5 py-2.5 rounded-xl hover:opacity-90 transition-opacity"
        >
