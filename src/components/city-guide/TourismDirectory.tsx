...
export default function TourismDirectory({ places }: TourismDirectoryProps) {
  ...
  return (
    <main className="bg-[#f5f2eb] pb-20 text-stone-900 dark:bg-zinc-950 dark:text-zinc-50">
      <section className="overflow-hidden border-b border-stone-300 bg-[#f5f2eb] dark:border-zinc-800 dark:bg-zinc-900">
        <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[1.15fr_.85fr] lg:px-8 lg:py-20">
          <div className="relative z-10">
            <p className="text-xs font-bold uppercase tracking-[.24em] text-[#8f6900] dark:text-yellow-400">Gorontalo City Guide</p>
            <h1 className="mt-4 max-w-2xl font-display text-4xl font-bold leading-[.98] tracking-[-.05em] sm:text-6xl">Jelajahi Gorontalo, lebih dekat.</h1>
          ...
          <div className="relative min-h-72 overflow-hidden rounded-[.7rem] bg-stone-950 shadow-[0_28px_60px_-34px_rgba(23,23,23,.6)] dark:bg-black lg:min-h-full">
            ...
            <div className="absolute inset-x-6 bottom-6 border border-white/20 bg-stone-950/90 p-5 text-white shadow-lg backdrop-blur">
              <p className="text-xs font-bold uppercase tracking-[.18em] text-[#f5c400]">Jelajah dengan santai</p>
      ...
      <section className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 lg:py-14">
        <div className="flex flex-col gap-5 border-b border-stone-200 pb-6 dark:border-zinc-800 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <p className="text-xs font-bold uppercase tracking-[.2em] text-[#8f6900] dark:text-yellow-400">Direktori tempat</p>
...
function SearchPanel({ query, setQuery, category, setCategory, categories, location, setLocation, locations }: {
  ...
  return <div className="mt-8 border border-stone-300 bg-white p-3 shadow-xl shadow-stone-900/5 dark:border-zinc-700 dark:bg-zinc-950">
    <div className="grid gap-2 md:grid-cols-[1fr_11rem_11rem]">
      <label className="flex min-h-12 items-center gap-3 bg-stone-50 px-4 dark:bg-zinc-900">
...
function FilterSelect({ value, onChange, options, label }: { value: string; onChange: (value: string) => void; options: string[]; label: string }) {
  return <label className="relative flex min-h-12 items-center bg-stone-50 px-3 dark:bg-zinc-900"><span className="sr-only">{label}</span><select value={value} onChange={(event) => onChange(event.target.value)} className="h-full w-full appearance-none bg-transparent pr-6 text-sm font-medium outline-none"><>{options.map((option) => <option key={option} value={option}>{option}</option>)}</></select><span className="pointer-events-none absolute right-3 text-xs text-slate-500">⌄</span></label>;
}

function PlaceCard({ place, rank }: { place: TourismPlace; rank: number }) {
  return <Link href={`/wisata/${place.slug}`} className="group overflow-hidden rounded-[.65rem] border border-stone-300 bg-white shadow-sm transition duration-300 hover:-translate-y-1 hover:border-stone-950 hover:shadow-xl hover:shadow-stone-900/10 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:border-yellow-400">
    <div className="relative aspect-[16/10] overflow-hidden bg-gradient-to-br from-amber-200 via-orange-100 to-stone-200 dark:from-amber-800 dark:to-zinc-800">
      ...
      <div className="absolute left-3 top-3 bg-[#f5c400] px-3 py-1 text-[11px] font-bold text-stone-950 shadow-sm">{place.featured ? "Pilihan City Guide" : `Rekomendasi #${rank}`}</div>
    ...
    <div className="p-5">
      <p className="text-[11px] font-bold uppercase tracking-[.16em] text-[#8f6900] dark:text-yellow-400">{place.category || "Wisata"}</p>
