...
export default async function TourismDetail({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-[#f5f2eb] pb-20 text-stone-900 dark:bg-zinc-950 dark:text-zinc-100">
    <div className="mx-auto max-w-7xl px-4 pt-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-[minmax(0,1fr)_21rem] lg:items-end">
        <div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#8f6900] dark:text-yellow-400">{category}{place.location ? ` · ${place.location}` : ""}</p><h1 className="mt-3 max-w-4xl font-display text-4xl font-bold leading-tight tracking-[-.05em] sm:text-6xl">{place.name}</h1><div className="mt-5 flex flex-wrap gap-2">{highlights.map((highlight) => <span key={highlight} className="border border-stone-300 bg-white px-3 py-1.5 text-sm text-slate-600 dark:border-zinc-700 dark:bg-zinc-900 dark:text-zinc-300">{highlight}</span>)}</div></div>
        <div className="border border-stone-300 bg-stone-950 p-5 text-white dark:border-zinc-700"><p className="text-xs font-bold uppercase tracking-[.18em] text-[#f5c400]">Simpan untuk perjalanan</p><p className="mt-2 text-sm leading-relaxed text-stone-300">Cek jam operasional dan rute sebelum berkunjung agar pengalamanmu lebih nyaman.</p></div>
      </div>
    </div>

    <div className="mx-auto mt-9 max-w-7xl px-4 sm:px-6 lg:px-8"><div className="relative aspect-[16/8] overflow-hidden rounded-[.65rem] bg-gradient-to-br from-amber-200 via-orange-100 to-stone-200 shadow-lg shadow-stone-900/10 dark:from-amber-900 dark:to-zinc-800">{place.image_url ? <Image src={place.image_url} alt={place.name} fill priority unoptimized className="object-cover" /> : <div className="absolute inset-0 bg-[radial-gradient(circle_at_70%_30%,rgba(255,255,255,.75),transparent_13%),linear-gradient(135deg,transparent_0_50%,rgba(120,53,15,.13)_50.5%_51%,transparent_51.5%)]" />}</div>{gallery.length > 1 && <div className="mt-3 grid grid-cols-4 gap-3">{gallery.slice(1, 5).map((image, index) => <div key={image} className="relative aspect-[4/3] overflow-hidden rounded-[.45rem] bg-stone-200 dark:bg-zinc-800"><Image src={image} alt={`${place.name} ${index + 2}`} fill unoptimized className="object-cover" /></div>)}</div>}<div className="mt-4 flex flex-wrap gap-3">{place.maps_url && <a href={place.maps_url} target="_blank" rel="noreferrer" className="bg-stone-950 px-4 py-2.5 text-sm font-semibold text-white hover:bg-stone-700 dark:bg-yellow-400 dark:text-zinc-950">Buka peta</a>}{place.website_url && <a href={place.website_url} target="_blank" rel="noreferrer" className="border border-[#d2a600] bg-white px-4 py-2.5 text-sm font-semibold text-[#725400] hover:bg-yellow-50 dark:border-yellow-400 dark:bg-zinc-900 dark:text-yellow-200">Website</a>}</div></div>
