...
export async function POST(req: NextRequest) {
  ...
  const { data, error } = await auth.supabase.from("tourism_places").insert({ ...(await req.json()), author_id: auth.user.id }).select().single();
...
export async function PATCH(req: NextRequest) {
  ...
  const { data, error } = await auth.supabase.from("tourism_places").update(patch).eq("id", id).select().single();
  return error ? NextResponse.json({ error: error.message }, { status: 400 }) : NextResponse.json({ data });
}
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = permanent
    ? await auth.supabase.from("tourism_places").delete().eq("id", id)
    : await auth.supabase.from("tourism_places")
